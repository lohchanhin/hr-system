...
根據問題描述：
- ✅ **測試資料要包含有員工是夜班**
- ✅ **月薪資總覽的資料包含夜班津貼資訊**（注意：夜班津貼屬於銀行B，不在總覽表格中顯示，但在詳細資料中可見）
...
**夜班津貼顯示**:
- **資料結構**: API 返回的資料中包含 `nightShiftAllowance` 欄位
- **顯示位置**: 夜班津貼屬於銀行 B（獎金帳戶），因此不在月薪資總覽表格中顯示為獨立欄位
- **詳細查看**: 點擊員工的「詳細」按鈕後，可以在夜班統計卡片和薪資計算明細中看到完整的夜班津貼資訊
...
**檢查步驟**:
1. 確認 API 回應是否包含 `nightShiftAllowance` 欄位
2. 在員工詳細資料中檢查夜班統計卡片和銀行B金額
3. 檢查瀏覽器控制台是否有錯誤訊息

**注意**: 夜班津貼屬於銀行 B（獎金帳戶），因此不在月薪資總覽表格中顯示為獨立欄位，但資料結構中仍包含此欄位，並在員工詳細資料中顯示。
...
✅ **月薪資總覽包含夜班津貼資訊**
- API 返回完整的 `nightShiftAllowance` 資料
- 夜班津貼屬於銀行 B（獎金帳戶），在員工詳細資料中顯示
- 支援動態計算和固定設定兩種模式
- 顯示格式化的金額和統計資訊
