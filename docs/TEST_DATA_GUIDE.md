...
- **夜班**：22:00 - 06:00（休息 02:00-03:00）- 跨日班，含夜班津貼 × 0.34

#### 夜班員工設定
為了測試夜班津貼功能，系統會將部分員工設定為夜班員工：
- **夜班員工**：偶數索引的員工（王小明、陳俊宏、吳建國）
- **固定夜班津貼**：2,000 - 4,000 元
- **排班分配**：夜班員工約 80% 的排班會被分配到夜班
...
#### 固定獎金項目
- 夜班津貼（根據實際夜班排班自動計算，或使用固定值 2,000-4,000 元）
- 績效獎金（2,000-8,000 元）
- 其他獎金（1,000-5,000 元）

#### 夜班津貼計算方式
對於有夜班排班的員工，系統會自動計算夜班津貼：
```
夜班津貼 = 時薪 × 夜班工作時數 × 津貼倍數 (0.34)
例如：時薪 200 元 × 7 小時 × 0.34 = 476 元/天
```
如果沒有夜班排班，則使用員工設定的固定津貼。
...
4. **夜班津貼計算**
   - 檢查夜班員工的薪資是否包含夜班津貼
   - 驗證夜班天數和時數是否正確
   - 確認津貼計算方式（calculated 表示根據排班計算，fixed 表示使用固定值）
   - 津貼金額 = 時薪 × 夜班工作時數 × 津貼倍數
...
#### 案例 5：夜班員工的薪資計算
- 選擇一個夜班員工（例如：王小明、陳俊宏或吳建國）
- 查看其上月的薪資記錄
- 驗證：
  - 夜班天數 (nightShiftDays) 是否正確
  - 夜班時數 (nightShiftHours) 是否正確
  - 夜班津貼金額是否合理
  - 計算方式 (nightShiftCalculationMethod) 應為 "calculated"
  - 手動計算：時薪 × 夜班工作時數 × 0.34 × 天數，比對系統計算結果
